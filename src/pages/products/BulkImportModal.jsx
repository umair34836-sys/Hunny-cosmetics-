...
const PLACEHOLDER = `name,category,brand,sku,unit,costPrice,sellingPrice,quantity,lowStockThreshold,expiryDate,supplier
Matte Lipstick,Lips,,,pcs,70,70,1,1,,
Facewash Cleanser,Skincare,,,pcs,100,100,1,1,,`
  ...
  return (
    <Modal open={open} onClose={handleClose} title={t('products.bulkImport')} size="xl">
      <div className="space-y-4">
        <p className="text-sm text-ink-muted">{t('products.bulkImportHelp')}</p>
                    ...
                    <tbody>
                      {parsed.rows.map((r, i) => (
                        <tr key={i} className="border-b border-surface-border last:border-0">
                          ...
                          <td className="px-3 py-1.5 text-ink-muted">{r.category || '—'}</td>
                          ...
                          <td className="px-3 py-1.5 text-ink-muted">{r.quantity} {r.unit}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-surface-border pt-4">
          ...
          >
            {importing ? `${t('common.saving')} (${progress}/${parsed?.rows.length})` : t('products.importAll', { n: parsed?.rows.length || 0 })}
          </Button>
        </div>
      </div>
    </Modal>
